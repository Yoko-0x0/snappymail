...
window.identiconSvg = (hash, txt, font) => {
	...
	// Select color based on hash - use last 7 chars or entire hash if shorter
	const hashStr = hash.toString();
	const hashPart = hashStr.length >= 7 ? hashStr.substr(-7) : hashStr;
	const colorIndex = parseInt(hashPart, 16) % googleColors.length;
	const color = googleColors[colorIndex] || googleColors[0];
